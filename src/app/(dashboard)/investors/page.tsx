...
function getTypeStyle(type: string | null) {
  if (!type) return { bg: '#E6E4DE', text: '#5F5E5A' }
  return TYPE_COLORS[type] || { bg: '#E6E4DE', text: '#5F5E5A' }
}
...
export default function InvestorsPage() {
  ...
  return (
    <div style={{ padding: '2rem 2.5rem', background: '#F2F0EB', minHeight: '100vh', fontFamily: 'var(--font-jakarta), sans-serif' }}>
      ...
      <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', marginBottom: '1.25rem' }}>
        {TYPE_FILTERS.map((t) => (
          <button
            ...
            style={{ padding: '5px 12px', fontSize: 12, fontWeight: activeType === t ? 600 : 500, borderRadius: 20, border: '1px solid', borderColor: activeType === t ? '#1A1814' : '#C4C2BA', background: activeType === t ? '#1A1814' : 'transparent', color: activeType === t ? '#F2F0EB' : '#5F5E5A', cursor: 'pointer', fontFamily: 'inherit' }}
          >
            {t}
          </button>
        ))}
      ...
      {/* Column headers */}
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 80px 120px 1fr', gap: '0 16px', padding: '8px 0', borderBottom: '1.5px solid #C4C2BA' }}>
      ...
      {/* Rows */}
      {loading ? (
        <div style={{ padding: '3rem 0', textAlign: 'center', color: '#888780', fontSize: 14 }}>Loading investors…</div>
      ) : error ? (
        <div style={{ padding: '3rem 0', textAlign: 'center', color: '#993C1D', fontSize: 14 }}>Could not load investors: {error}</div>
      ) : paginated.length === 0 ? (
        <div style={{ padding: '3rem 0', textAlign: 'center', color: '#888780', fontSize: 14 }}>No investors found.</div>
      ) : (
        paginated.map((investor) => {
          ...
          return (
            <div
              key={investor.id}
              style={{ display: 'grid', gridTemplateColumns: '2fr 80px 120px 1fr', gap: '0 16px', padding: '12px 0', borderBottom: '0.5px solid #DDD9D3', alignItems: 'center', borderRadius: 4 }}
            ...
            >
                ...
                <div style={{ minWidth: 0 }}>
                  ...
                  {displayUrl && (
                    <a
                      ...
                      style={{ fontSize: 11, color: '#B4B2A9', textDecoration: 'none', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', display: 'block', maxWidth: 220 }}
                    >
                      {displayUrl}
                    </a>
                  )}
              ...
              {/* Country */}
              <span style={{ fontSize: 13, fontWeight: 500, color: investor.hq_country ? '#1A1814' : '#B4B2A9' }}>
                {investor.hq_country || '—'}
              </span>

              {/* Focus sectors */}
              <span style={{ fontSize: 12, color: investor.focus_sectors ? '#5F5E5A' : '#B4B2A9', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
